<template>
  <div>
    ...
    >
      <b-navbar-nav>
        <b-nav-item @click="$router.push('/').catch((err) => {})"
          ><i class="fa fa-fw fa-home"></i>Home</b-nav-item
        >
        <b-nav-item
          v-if="!token"
          ...
          <b-form class="form-inline my-2 my-lg-0">
            ...
            <div class="dwqd" v-else>
              <div class="siema" v-if="errors.length">
                <H4 v-for="(error, index) in errors" v-bind:key="error + index"
                  >{{ error }}
              ...
              <div v-else class="roll">
                ...
                <button
                  ...
                  @click="zaloguj"
                >
                  <i class="fa fa-fw fa-user"></i> Log in
                </button>
              </div>
            </div>
            <button
...
export default {
  ...
  mounted() {
    if (this.$cookies.get("Token"))
      this.success.push("Witaj " + this.$cookies.get("login") + "!");
    var cook = [];
